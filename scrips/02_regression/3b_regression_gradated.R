...
linear_model = LDN_grid$count_AEDs_gradated ~ LDN_grid$pc_f.scaled +
...
model_1 <- LDN_grid$count_AEDs_gradated ~
...
model_2 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  LDN_grid$pc_65_plus.scaled +
  #LDN_grid$pc_50_plus.scaled +
  LDN_grid$pc_bad_gh.scaled +
...
model_3 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  #LDN_grid$pc_65_plus.scaled +
  LDN_grid$pc_50_plus.scaled +
  LDN_grid$pc_bad_gh.scaled +
...
model_4 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  LDN_grid$pc_65_plus.scaled +
  LDN_grid$pc_50_plus.scaled +
  #LDN_grid$pc_bad_gh.scaled +
...
model_5 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  LDN_grid$pc_65_plus.scaled +
  LDN_grid$pc_50_plus.scaled +
  LDN_grid$pc_bad_gh.scaled +
...
model_6 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  LDN_grid$pc_65_plus.scaled +
  LDN_grid$pc_50_plus.scaled +
  LDN_grid$pc_bad_gh.scaled +
...
model_7 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  LDN_grid$pc_65_plus.scaled +
  LDN_grid$pc_50_plus.scaled +
  LDN_grid$pc_bad_gh.scaled +
...
model_9 <- LDN_grid$count_AEDs_gradated ~
  LDN_grid$pc_f.scaled +
  LDN_grid$pc_65_plus.scaled +
  #LDN_grid$pc_50_plus.scaled +
  LDN_grid$pc_bad_gh.scaled +
...
for (n_model in 1:(length(models))){
  ...
  for(n_Amat in 1:(length(A_matrices))){
    ...
    filename = paste("outputs/02_regression/data/regression_results_gradated/", unique_model_name,
                     ".csv", sep="")
    ...
    write.table(model_ranking, "outputs/02_regression/data/model_ranking_gradated.csv",
...
LDN_grid$AED_demand_gradated <- car_output$fit$fitted.values


plot_descriptive_ldn("AED_demand",title = "AED Demand",
                     legend_title = "Number of AEDs", map = LDN_grid,
                     cap=TRUE, max_val = 3, bins=5)
plotRegresssion("AED_demand_gradated",title = "AED Demand", map = LDN_grid,
                max_relevant_val = 15)


# Saving Model ------------------------------------------------------------

write_sf(LDN_grid, "outputs/02_regression/data/regressed_gradated_data_ldn.gpkg")
